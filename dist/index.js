...
let DxButton = class DxButton extends s {
    ...
    render() {
        return x `<button @click="${this.onClick}"><slot></slot></button>`;
    }
};
DxButton.styles = [
    GlobalStyles,
...
__decorate([
    e({ attribute: 'outline', reflect: true })
], DxButton.prototype, "outline", void 0);
DxButton = __decorate([
    e$1('dx-button')
], DxButton);
...
let DxBreadcrumb = class DxBreadcrumb extends s {
    ...
    renderList(data) {
        const items = data.map((item, i) => {
            ...
            const label = !isLast && item.link
                ? x `<a href="${item.link}">${item.title}</a>`
                : item.title;
            return x `<li><span>${label}</span></li>`;
        });
        return x `<ul>
      ${items}
    </ul>`;
    ...
    render() {
        return x ` <div>${this.renderList(this.data)}</div> `;
    }
};
DxBreadcrumb.styles = [
    GlobalStyles,
...
let DxDateInfo = class DxDateInfo extends s {
    ...
    render() {
        return x `<span>${this.parseValue()}</span>`;
    }
};
